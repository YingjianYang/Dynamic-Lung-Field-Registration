...
from acregnet.datasets import ImagePairsDatasetTrain_V4
...
if __name__ == "__main__":
    ...
    train_dataset = ImagePairsDatasetTrain_V4(args.train_images_file, args.train_labels_file)
    ...
    # 使用v3训练好的模型进行网络参数初始化
    model.load('../results/DRLung/ACRegNet/train/model_v3.pt')
    ...
    for epoch in trange(args.epochs):
        ...
        if args.save_model:
            model.save(os.path.join(args.results_dir, 'model_v4.pt'))
    ...
    save_dict(train_results, os.path.join(args.results_dir, 'train_results_v4.pkl'))

    with open(os.path.join(args.results_dir, 'run_args_v4.txt'), 'w') as f:
        for k, v in sorted(vars(args).items()):
            f.write(f'{k}: {v}\n')
